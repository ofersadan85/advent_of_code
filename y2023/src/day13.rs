            ...
            match (diff_count, smudge_found) {
                (0, _) => {
                    info!(
                        ...
                        inspected_index + 1,
                        reflected_index + 1,
                        lines[inspected_index]
                    );
                    if inspected_index == 0
                        || reflected_index == lines.len() - 1
                ...
                (1, false) => {
                    ...
                    info!("There's one smudge at index {}", index + 1);
                    smudge_found = true;
                    if inspected_index == 0
                        || reflected_index == lines.len() - 1
                            && previous_value != Some(index + 1)
...
fn box_value(input: &str, smudged: bool) -> usize {
    ...
    match value {
        ...
        None => {
            info!("No horizontal reflection found");
...
mod tests {
    ...
    #[test]
    fn part2() {
        assert_ne!(mirrors(include_str!("day13.txt"), true), 31110);
        assert!(mirrors(include_str!("day13.txt"), true) > 36597);
    }
}
